...
def serve_extension():
    ...
    text = data['text'] # text holds full blurb to get acronym disambiguations
    print("request received!")
    use_zeroshot = data['use_zeroshot']

    acronym_predictions, tokens = acronym_predictor.predict(text, use_zeroshot)
    
    print('acronym_predictions: ', acronym_predictions)
    print('tokens: ', tokens)

    return {"acronym_predictions": acronym_predictions, "tokens": tokens}
    

if __name__ == '__main__':
    app.run(host='0.0.0.0', port='5000') # try debug=True
